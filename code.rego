...
allow if {
    ...
    allowed := billing_allowed_fields[input.context.table]
    every field in input.context.fields {
        field in allowed
    }
}
...
allow if {
    ...
    allowed := insurance_allowed_fields[input.context.table]
    every field in input.context.fields {
        field in allowed
    }
}
...
allow if {
    ...
    allowed := case_manager_allowed_fields[input.context.table]
    every field in input.context.fields {
        field in allowed
    }
}